...
test("fresh user-only installation is idempotent and preserves existing files", () => {
  ...
  assert.ok(fs.existsSync(path.join(codexHome, "dispatcher", "src", "cli.mjs"))); assert.ok(fs.existsSync(path.join(root, "space home", ".local", "bin", "codex-worker")));
  const stateFile = path.join(codexHome, "state", "codex-dispatcher.json"); const manifest = JSON.parse(fs.readFileSync(stateFile, "utf8")); const worker = path.join(root, "space home", ".local", "bin", "codex-worker");
  assert.equal(manifest.version, 5); assert.equal(manifest.hashes_after[worker], crypto.createHash("sha256").update(fs.readFileSync(worker)).digest("hex")); assert.equal(fs.statSync(stateFile).mode & 0o077, 0);
});
...
test("second CLI install stdout excludes preserved baseline content", () => {
  ...
  assert.match(manifest.user_preservations[path.join(codexHome, "config.toml")].content, new RegExp(secret)); assert.equal(fs.statSync(stateFile).mode & 0o077, 0);
...
test("v1.2 updates retain user-modified private release state dormant and owner-only", () => {
  ...
  install({ home: root, codexHome, runner: fakeRunner }); const next = JSON.parse(fs.readFileSync(stateFile, "utf8")); assert.equal(next.retired_legacy_release_state, false); assert.equal(fs.existsSync(releaseFile), true); assert.equal(fs.statSync(releaseFile).mode & 0o077, 0);
});
